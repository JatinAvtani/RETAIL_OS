...
/**
 * real HTTP verification for `integrations.reconcileSquareOrders`. The router no longer runs
 * the sweep itself — it does the same preconditions as `syncSquareOrders` (connection exists,
 * linked location) and enqueues a real BullMQ job onto `squareSyncQueue`, returning
 * `{ enqueued: true }` immediately. The real reconciliation behavior (finding orders the
 * incremental watermark would never revisit, never perturbing that watermark, refund detection,
 * idempotency) is now proven directly against `reconcileSquareOrders` in
...
describe('integrations.reconcileSquareOrders', () => {
  ...
  it('a genuinely connected store enqueues a real reconcile job and returns immediately', async () => {
    const { organizationId, storeId } = await setUpOrgWithStore();
    ...
    expect(ourJob!.data.kind).toBe('reconcile');
    expect(ourJob!.name).toBe('reconcile');
  });
});
