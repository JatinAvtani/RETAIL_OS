...
export const syncSquareOrders = async (
  ...
  // from the last order actually seen.
  const syncStartedAt = new Date();

  let cursor = connection.ordersSyncCursor ?? undefined;
  let ordersSeen = 0;
  ...
  try {
    do {
      const page = await fetchSquareOrders(config, accessToken, connection.externalLocationId, since, cursor);
        ...
        withTenantContext(tx, organizationId, async () => {
          ...
          await tx
            ...
            .set({
              ...
              ordersSyncWatermark: page.nextCursor === undefined ? syncStartedAt : connection.ordersSyncWatermark,
              updatedAt: new Date(),
            })
