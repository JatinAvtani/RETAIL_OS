...
/**
 * The transactional outbox (I8, the design): a state change and its event MUST commit together
 ...
 * Redis — no such relay is built yet (out of scope for earlier work, confirmed with the user: this table
 ...
 * accumulating safely if Redis is down is itself part of the design.
 *
 * Not partitioned — the design's partitioning table does not list `outbox_events`, unlike
 ...
 * permanent record.
 *
 * `aggregateType`/`aggregateId`/`eventType`/`payload` match the design's literal example
