...
/**
 * earlier work (the scope's M3 milestone: "Par levels and reorder points per product per store"):
 * pure data storage, confirmed with the user. The full deterministic reorder-suggestion FORMULA
 * is a separate,
 * later task that reads `reorderPoint` as
 * one input among several; this table does not compute anything, and `stock.below_par` event
 * emission/alerting is likewise out of scope here (not in the later milestone's own task list, same
 * detection-only precedent as earlier work's `findStockLevelDrift`).
 *
 * `parLevel` and `reorderPoint` are separate columns because they answer different questions: par
 * level is "how much should be on hand at a glance" (the design's fallback for a brand-new
 * product with no consumption history yet — "use par level only"), reorder point is "the threshold
 * that triggers a reorder" (the design's `daily_consumption × lead_time + safety_stock`, a
